...
- **Repository:** apaperclip/hass-sdcp

**Key directories:**

- `custom_components/sdcp/` - Main integration code
...
- Integration logs: `custom_components.sdcp: debug` in `config/configuration.yaml`
...
- Use the domain `sdcp` for all DOMAIN references
- Prefix all integration-specific classes with `SDCPHomeAssistant`
- Use "SDCP for Home Assistant" as the display title
...
- Inherit from platform base + `SDCPHomeAssistantEntity`
...
- `tests/` mirrors `custom_components/sdcp/` structure
