...
from custom_components.sdcp.config_flow_handler.schemas import (
    get_reauth_schema,
    get_reconfigure_schema,
    get_user_schema,
)
from custom_components.sdcp.config_flow_handler.validators import validate_credentials
from custom_components.sdcp.const import DOMAIN, LOGGER
from homeassistant import config_entries
from homeassistant.const import CONF_HOST
from homeassistant.loader import async_get_loaded_integration

if TYPE_CHECKING:
    from custom_components.sdcp.config_flow_handler.options_flow import SDCPHomeAssistantOptionsFlow

# Map exception types to error keys for user-facing messages
ERROR_MAP = {
    "SDCPHomeAssistantApiClientAuthenticationError": "auth",
    "SDCPHomeAssistantApiClientCommunicationError": "connection",
}


class SDCPHomeAssistantConfigFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    """
    Handle a config flow for sdcp.
    ...
    def async_get_options_flow(
        ...
        """
        from custom_components.sdcp.config_flow_handler.options_flow import (  # noqa: PLC0415
            SDCPHomeAssistantOptionsFlow,
        )

        return SDCPHomeAssistantOptionsFlow()
        ...
        if user_input is not None:
            try:
                ...
                return self.async_create_entry(
                    title=f"SDCP Projector ({user_input[CONF_HOST]})",
                    data=user_input,
                )
    ...
    async def async_step_reconfigure(
        ...
        return self.async_show_form(
            step_id="reconfigure",
            data_schema=get_reconfigure_schema(entry.data.get(CONF_HOST, "")),
            errors=errors,
        )
    ...
    async def async_step_reauth_confirm(
        ...
        return self.async_show_form(
            ...
            data_schema=get_reauth_schema(entry.data.get(CONF_HOST, "")),
            errors=errors,
            description_placeholders={
...
__all__ = ["SDCPHomeAssistantConfigFlowHandler"]
