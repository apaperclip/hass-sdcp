"""
Core DataUpdateCoordinator implementation for sdcp.
...
from custom_components.sdcp.api import SDCPHomeAssistantApiClientAuthenticationError, SDCPHomeAssistantApiClientError
from custom_components.sdcp.const import LOGGER
...
class SDCPHomeAssistantDataUpdateCoordinator(DataUpdateCoordinator):
    """
    ...
    config_entry: SDCPHomeAssistantConfigEntry
    ...
    async def _async_setup(self) -> None:
        ...
        if not self.config_entry.runtime_data.client.is_connected:
            await self.config_entry.runtime_data.client.async_connect()
    ...
    async def _async_update_data(self) -> Any:
        ...
        The API client uses the host from config_entry to connect:
        - host: from config_entry.data["host"]

        Expected API response structure (example):
        {
            "power": "standby",      # Power status string
            "lamp_hours": 1234,      # Lamp runtime in hours
        ...
        except SDCPHomeAssistantApiClientAuthenticationError as exception:
            LOGGER.warning("Authentication error - %s", exception)
            raise ConfigEntryAuthFailed(
                translation_domain="sdcp",
                translation_key="authentication_failed",
            ) from exception
        except SDCPHomeAssistantApiClientError as exception:
